...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const evidenceSeverity = document.getElementById("evidence-severity");
    const evidenceConfidence = document.getElementById("evidence-confidence");
    ...
    const routeCondition = document.getElementById("route-condition");
    const priorityFill = document.getElementById("priority-fill");
    const routePriority = document.getElementById("route-priority");
    const priorityReason = document.getElementById("priority-reason");

    const routeRisk = document.getElementById("route-risk");
    ...
    const responseStep1 = document.getElementById("response-step-1");
    const responseStep2 = document.getElementById("response-step-2");
    const responseStep3 = document.getElementById("response-step-3");
    ...
    function loadLensEvidence() {
        ...
        /*
         * LENS stores its latest inspection in sessionStorage.
         * We deliberately check several possible keys so that
         * PATHFINDER remains tolerant of small naming changes.
         */
        ...
        try {
            ...
            lensEvidence = JSON.parse(raw);

            if (!lensEvidence || typeof lensEvidence !== "object") {
                throw new Error("Invalid LENS evidence.");
            }

            populateEvidence(lensEvidence);

        } catch (error) {

            console.error(
                "[PATHFINDER] Could not parse LENS evidence:",
                error
            );
    ...
    function showWaitingState() {
        ...
        evidenceCard.classList.remove("is-loaded");

        evidenceState.textContent = "WAITING";

        condition.textContent = "—";
        evidenceSeverity.textContent = "SEVERITY —";
        evidenceConfidence.textContent = "CONFIDENCE —";

        locationValue.textContent = "—";
        locationCoordinates.textContent = "NO COORDINATES";
        ...
        systemState.textContent = "AWAITING EVIDENCE";
    }
    ...
    function populateEvidence(data) {
        ...
        evidenceCard.classList.add("is-loaded");

        evidenceState.textContent = "RECEIVED";
        systemState.textContent = "EVIDENCE RECEIVED";
        ...
        const description =
            ...
            data.observation ||
            data.details ||
            "Visual evidence received from LENS.";

        condition.textContent = normalize(defect);
        ...
        if (confidence !== null) {
            ...
            if (!Number.isNaN(confidenceNumber)) {
                ...
                evidenceConfidence.textContent =
                    `CONFIDENCE ${safeText(confidence)}`;
            }
        ...
        evidenceDescription.textContent =
            safeText(description, "Visual evidence received from LENS.");
    ...
    function deriveRoute(data) {
        ...
        let risk = "INFRASTRUCTURE HAZARD";
        ...
        if (
            ...
            risk = "VEHICLE / PEDESTRIAN HAZARD";
        ...
        let route = "INFRASTRUCTURE MAINTENANCE";
        ...
        return {
            condition: defect || "UNKNOWN CONDITION",
            priority,
            priorityScore,
            priorityReason:
            ...
            route,
            routeDescription: routeDescriptionText,
            response
        };
    }
    ...
    async function generateRoute() {
        ...
        if (!lensEvidence || routeGenerated) {
            return;
        }
        ...
        generateButton.disabled = true;
        generateButton.querySelector("span:first-child").textContent =
            "ROUTING EVIDENCE";

        systemState.textContent = "ROUTING";
        document.querySelector(".pf-header").classList.add("is-active");

        clearRouteState();

        const route = deriveRoute(lensEvidence);
        ...
        activateNode("response");

        routingSection.classList.add("is-generated");

        await sleep(500);

        renderSummary(route);

        summarySection.classList.add("is-generated");

        systemState.textContent = "ROUTE IDENTIFIED";

        generateButton.querySelector("span:first-child").textContent =
            "ROUTE GENERATED";

        document.querySelector(".pf-header").classList.remove("is-active");
    }
    ...
    function activateNode(nodeName) {
        ...
        if (
            previousLine &&
            previousLine.classList.contains("pf-route-line")
        ) {
            previousLine.classList.add("is-active");
        }
    }
    ...
    function clearRouteState() {
        ...
        document
            .querySelectorAll(".pf-route-node")
            .forEach(node => {
                node.classList.remove("is-active");
            });

        document
            .querySelectorAll(".pf-route-line")
            .forEach(line => {
                line.classList.remove("is-active");
            });

        routingSection.classList.remove("is-generated");
        summarySection.classList.remove("is-generated");

        priorityFill.style.width = "0%";
        ...
        responseStep1.textContent = "INSPECT";
        responseStep2.textContent = "REPAIR";
        responseStep3.textContent = "VERIFY";
